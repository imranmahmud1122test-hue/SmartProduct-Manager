...
export const OwnerOrdersView: React.FC<OwnerOrdersViewProps> = ({
  ...
  const processingCount = orders.filter(
    (o) => o.orderStatus === 'Confirmed' || o.orderStatus === 'Processing' || o.orderStatus === 'Ready' || o.orderStatus === 'Out for Delivery'
  ).length;
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    if (searchQuery.trim()) {
      ...
      const matchProduct = o.items.some((it) => it.productNameSnapshot.toLowerCase().includes(q));
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3.5 shadow-xs">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
          ...
          {['ALL', 'Pending', 'Confirmed', 'Processing', 'Ready', 'Out for Delivery', 'Delivered', 'Cancelled'].map((st) => {
                  ...
                  return (
                    <tr key={order.orderId} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3.5 px-4 align-top text-right space-x-1 whitespace-nowrap">
                        ...
                        <button
                          onClick={() => handlePrintInvoice(order)}
                          className="px-2.5 py-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors inline-flex items-center gap-1"
                        >
          ...
          <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col justify-between overflow-hidden animate-in slide-in-from-right duration-250 border-l border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                ...
                <div className="space-y-1.5">
                  <span className="text-[11px] font-semibold text-slate-500 block">Change Status to:</span>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    {(['Confirmed', 'Processing', 'Ready', 'Out for Delivery', 'Delivered', 'Cancelled'] as OrderStatus[]).map(
                      (st) => {
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-all text-center ${
                              isCurrent
                                ...
                                : 'bg-white border-slate-200 text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 hover:border-emerald-300'
                            }`}
                          >
                            {st}
                          </button>
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
              <button
                ...
                className="py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-xs transition-all flex items-center gap-1.5"
              >
              ...
              <button
                onClick={() => setSelectedOrder(null)}
                className="py-2.5 px-4 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl text-xs hover:bg-slate-100 transition-colors"
              >
                Close Drawer
              </button>
            </div>
