              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div className="flex items-center gap-2 max-w-[170px] overflow-hidden">
                  {business?.logoUrl ? (
                    ...
                    <div className="w-7 h-7 rounded-lg bg-emerald-600 text-white font-black text-xs flex items-center justify-center shadow-2xs shrink-0">
                      {business?.name ? business.name.charAt(0).toUpperCase() : (currentUser.businessName?.charAt(0).toUpperCase() || 'S')}
                    </div>
                  )}
                  <span className="font-extrabold text-sm text-slate-900 truncate">
                    {business?.name || currentUser.businessName || 'SPM Menu'}
                  </span>
                </div>
                <button
